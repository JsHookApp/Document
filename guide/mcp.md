...
| `mem_scan` | 通用内存值搜索（类似 Cheat Engine 首次扫描） |
| `mem_scan_filter` | 二次过滤扫描结果（类似 Cheat Engine 的 Next Scan） |
| `mem_scan_reset` | 重置/清除扫描会话 |
| `mem_scan_list` | 列出所有活跃的扫描会话 |

### 内存冻结
| 工具名 | 功能 |
|--------|------|
| `mem_freeze` | 冻结内存值（后台线程持续写入） |
| `mem_freeze_list` | 列出所有当前冻结的内存地址 |
| `mem_unfreeze` | 解除内存冻结 |
...
1. **编写执行** → `frida_execute` 执行 Frida 脚本
2. **查看日志** → `frida_read_log` 查看 console.log 输出
3. **迭代调试** → 根据日志调整脚本，重复执行
4. **保存脚本** → `frida_save_script` 将完成的脚本保存到设备
